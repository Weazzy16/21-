    ...
    public class Repository
    {
        ...
        public static void PutToObject(ExtPlayer player, InventoryItemData item, float posX, float posY, float posZ, float rotX, float rotY, float rotZ, bool isMy = false)
        {
            try
            {
                ...
                // ✅ СОЗДАЁМ УНИКАЛЬНОЕ ИМЯ ЛОКАЦИИ ДЛЯ ДРОПА
                string locationName = $"drop_{item.SqlId}_{DateTime.Now.Ticks}";
                string location = "drop";

                // ✅ ДОБАВЛЯЕМ ДРОП В ItemsData
                Chars.Repository.AddInventoryArray(locationName, location);
                Chars.Repository.ItemsData[locationName][location][0] = new InventoryItemData(
                    SqlId: item.SqlId,
                    ItemId: item.ItemId,
                    Count: item.Count,
                    Data: item.Data,
                ...
                // ✅ СОЗДАЁМ ЗАГЛУШКИ ДЛЯ МНОГОКЛЕТОЧНЫХ ПРЕДМЕТОВ
                if (itemInfo.Width > 1 || itemInfo.Height > 1)
                {
                    Chars.Repository.CreatePlaceholdersForSlot(player, locationName, location, 0, item.ItemId, item.IsTurn);
                }

                // ✅ СОХРАНЯЕМ locationName В dropData
                dropData.LocationName = locationName;
                ...
                var itemData = new Dictionary<string, object>
        {
            ...
            { "Width", itemInfo.Width }, // ✅ ДОБАВЛЕНО
    { "Height", itemInfo.Height }, // ✅ ДОБАВЛЕНО
    { "IsTurn", item.IsTurn } // ✅ ДОБАВЛЕНО
        };
                Log.Write($"[DROPDATA] ItemId={item.ItemId}, Width={itemInfo.Width}, Height={itemInfo.Height}, IsTurn={item.IsTurn}");
                ...
                Log.Write($"[PUTTODROP] Created drop: ItemId={item.ItemId}, LocationName={locationName}, Size={itemInfo.Width}x{itemInfo.Height}");
            }
            catch (Exception e)
            {
        ...
        private static void DeleteObject(GTANetworkAPI.Object obj)
        {
            try
            {
                ...
                {
                    var locationName = dropData.LocationName ?? "drop"; // ✅ ИСПОЛЬЗУЕМ locationName
                    var location = "drop";

                    // ✅ УДАЛЯЕМ ЗАГЛУШКИ
                    if (!string.IsNullOrEmpty(dropData.LocationName))
                    {
                        Chars.Repository.DeletePlaceholdersForSlot(null, dropData.LocationName, location, 0, forceDelete: true);
                    }

                    // ✅ УДАЛЯЕМ ИЗ ItemsData
                ...
                Log.Write($"[DELETEDROP] Deleted drop: LocationName={dropData.LocationName}");
            }
            catch (Exception e)
            {
        ...
        public static void ItemRaise(ExtPlayer player, GTANetworkAPI.Object obj)
        {
            try
            {
                ...
                if (count == 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(dropData.LocationName))
                    {
                        Chars.Repository.DeletePlaceholdersForSlot(null, dropData.LocationName, "drop", 0, forceDelete: true);
                    }

                    // ✅ УДАЛЯЕМ ИЗ ItemsData
                    if (Chars.Repository.ItemsData.ContainsKey(dropData.LocationName))
                    {
                        Chars.Repository.ItemsData.TryRemove(dropData.LocationName, out _);
                    }
