    ...
    class WeaponRepository : Script
    {
        ...
        public static void TakeWeapon(ExtPlayer player) // Пока что думаем как реализовать instant переменную.
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "cef.inventory.UpdateActiveWeapon", 0, 0);
        ...
        public static void RemoteEvent_changeWeapon(ExtPlayer player, int key)
        {
            try
            {
                ...
                // ✅ ЕСЛИ ЭТО ССЫЛКА — НАХОДИМ ОРИГИНАЛЬНЫЙ ПРЕДМЕТ
                if (Item.ItemId == ItemId.Debug && Item.Data != null && Item.Data.StartsWith("link_"))
                {
                    var parts = Item.Data.Split('_');
                    if (parts.Length >= 4 && int.TryParse(parts[1], out int originalSqlId))
                    {
                        string originalLocation = parts[2];
                        int originalIndex = int.Parse(parts[3]);

                        // ✅ БЕРЁМ ОРИГИНАЛЬНЫЙ ПРЕДМЕТ
                        Item = Chars.Repository.GetItemData(player, originalLocation, originalIndex);

                        if (Item.ItemId == ItemId.Debug)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Предмет не найден в инвентаре!", 3000);
                            return;
                        }
                    }
                    else
                ...
                if (ItemInfo.functionType == newItemType.Weapons || ItemInfo.functionType == newItemType.MeleeWeapons)
                {
                    ...
                    // ✅ ОЧИЩАЕМ ТОЛЬКО ЕСЛИ УБИРАЕМ ОРУЖИЕ (НАЖАЛИ НА ТУ ЖЕ КНОПКУ)
                    if (index == key)
                    {
                        Trigger.ClientEvent(player, "client.inventory.clearActiveWeapon");
                        return;
                    ...
                    else
                    {
                        if (Item.ItemId == ItemId.Snowball)
                            ...
                            Trigger.ClientEvent(player, "client.weapon.give", (int)wHash, 1, false, Item.ItemId);
                    }
                ...
                else
                {
                    ...
                    // ✅ ОЧИЩАЕМ ТОЛЬКО ЕСЛИ УБИРАЕМ ПРЕДМЕТ
                    if (index == key)
                    {
                        Trigger.ClientEvent(player, "client.inventory.clearActiveWeapon");
                        return;
                    ...
                    if (isUseItem == -1)
                        Chars.Repository.ItemsUse(player, "fastSlots", key, false);
                    else
                    {
                        BattlePass.Repository.UpdateReward(player, 99);
                    }
                ...
                if (isUseItem == 1)
                {
                    ...
                    Log.Write($"[WEAPON DEBUG] Sending setActiveWeapon: SqlId={Item.SqlId}, ItemId={Item.ItemId}, Data={Item.Data}, Index={key}");

                    Trigger.ClientEvent(player, "client.inventory.setActiveWeapon", Item.SqlId, (int)Item.ItemId, Item.Data, key);
