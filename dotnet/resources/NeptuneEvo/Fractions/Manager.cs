                    ...
                    if (vehicleLocalData != null)
                    {
                        ...
                        switch (action)
                        {
                            case "load_mats":
                                ...
                                Chars.Repository.AddNewItem(null, VehicleManager.GetVehicleToInventory(vehicle.NumberPlate), "vehicle", ItemId.Material, amount, MaxSlots: vMain.GetMaxWeight(vehicle.Model));
                            ...
                            case "load_drugs":
                                ...
                                if (count >= (vMain.GetMaxWeight(vehicle.Model) * Chars.Repository.ItemsInfo[ItemId.Drugs].Stack))
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.CarMaxDrugs), 3000);
                                    return;
                                }
                                else if ((count + amount) > (vMain.GetMaxWeight(vehicle.Model) * Chars.Repository.ItemsInfo[ItemId.Drugs].Stack))
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.InCarSpace, (vMain.GetMaxWeight(vehicle.Model) * Chars.Repository.ItemsInfo[ItemId.Drugs].Stack) - count), 3000);
                                    return;
                                }
                                Chars.Repository.AddNewItem(null, VehicleManager.GetVehicleToInventory(vehicle.NumberPlate), "vehicle", ItemId.Drugs, amount, MaxSlots: vMain.GetMaxWeight(vehicle.Model));
                            ...
                            case "load_medkits":
                                ...
                                if ((Chars.Repository.getCountItem(VehicleManager.GetVehicleToInventory(vehicle.NumberPlate), ItemId.HealthKit) + amount) > vMain.GetMaxWeight(vehicle.Model))
                                {
                                ...
                                Chars.Repository.AddNewItem(null, VehicleManager.GetVehicleToInventory(vehicle.NumberPlate), "vehicle", ItemId.HealthKit, amount, MaxSlots: vMain.GetMaxWeight(vehicle.Model));
                                fractionData.MedKits -= amount;
