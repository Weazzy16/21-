...
namespace NeptuneEvo.Fractions
{
        ...
        public static Dictionary<uint, string> weaponHashToString = new Dictionary<uint, string>
    {
        ...
        private static string GetWeaponName(uint weaponHash)
        {
            if (weaponHashToString != null && weaponHashToString.TryGetValue(weaponHash, out var name))
                return name;
        ...
        [RemoteEvent("deathScreen.deathButton")]
        public void OnDeathScreenDeathButton(ExtPlayer player, string action, string placeType)
        {
            try
            {
                Log.Write($"OnDeathScreenDeathButton: {action} {placeType}");
                // Сохраните выбор: sessionData.SelectedSpawn = placeType;
                // Если action требует немедленного вызова EMS/таймера — вызовите соответствующую логику:
                if (action == "callMedics")
                {
                    OnCallEms(player, true); // или ваша логика
                }
                else if (action == "loseConsciousness")
                {
                    // например, отправить в больницу или старт таймера
                    OnHospitalDialogCallback(player, 3); // если хотите использовать старую логику
                }
            }
            catch (Exception e)
            {
                Log.Write($"OnDeathScreenDeathButton exception: {e}");
            }
        }
        // Вставьте этот фрагмент в класс Ems (заменяет существующий метод OnPlayerDeathHandler).
        // Убедитесь, что в файле есть using Newtonsoft.Json; (вверху файла) для сериализации JSON.
        ...
        public void OnPlayerDeathHandler(ExtPlayer player, ExtPlayer entityKiller, uint weapon)
        {
            try
            {
                ...
                // Если была сохранённая информация о киллере в sessionData (например, из логики), используем её
                ...
                // Вызов внешней логики администратора/статусов смерти
                Admin.onPlayerDeathHandler(player, entityKiller, weapon, player.Position);

                // Сбрасываем прицел/биноки если были
                ...
                try
                {
                    if (entityKiller != null && entityKiller.IsCharacterData()
                        && Events.AirDrop.Repository.IsPlayerToEvents(player)
                ...
                var killerCharacterData = entityKiller != null ? entityKiller.GetCharacterData() : null;
                var killerSessionData = entityKiller != null ? entityKiller.GetSessionData() : null;
                ...
                // Специальные режимы — если игрок в специальных лобби/событиях, передаём управление туда
                ...
                if (sessionData.InAirsoftLobby >= 0)
                {
                    if (player.GetSharedData<int>("PlayerAirsoftTeam") == 0
                        ...
                        || (Events.Airsoft.AirsoftPlayerData.ContainsKey(player) && Events.Airsoft.AirsoftPlayerData[player].IsGunGamePlayer == true))
                    {
                ...
                if (CarRoom.OnExitTestDrive(player, isDeath: true))
                {
                ...
                // Общая игровая логика при смерти
                ...
                // Если игрок в деморгане или под арестом — мгновенное возрождение/ремув
                ...
                // Основная ветка — нормальная смерть
                NAPI.Player.SpawnPlayer(player, player.Position);

                if (!sessionData.DeathData.IsDying)
                {
                    // Подтверждение смерти и перевод в состояние InDeath
                    DeathConfirm(player);
                    ...
                    // Формируем структурированные данные для нового popup (JSON), чтобы Svelte-попап получал places и deathScreenData
                    var killerName = (entityKiller != null && entityKiller.IsCharacterData()) ? entityKiller.Name : "";
                    var killerId = (entityKiller != null && entityKiller.IsCharacterData()) ? entityKiller.Value : 0;
                    var killerExists = (entityKiller != null && entityKiller.IsCharacterData() && entityKiller != player);

                    // Получение имени оружия по хешу (если есть)
                    string weaponName = "";
                    if (killerExists)
                    {
                    ...
                    // Пример мест возрождения (places). Подставьте реальные spawn-плейсы при необходимости.
                    var places = new[] {
                new { type = "hospital", name = "Больница", hasOrganization = false },
                new { type = "mansion_1", name = "Особняк A", hasOrganization = true }
            };

                    double distance = 0;
                    if (killerExists)
                    {
                        try { distance = Math.Round(player.Position.DistanceTo(entityKiller.Position), 1); } catch { distance = 0; }
                    }

                    var deathScreenData = new
                    {
                        active = true,
                        selfRevive = false,
                        killerNone = !killerExists,
                    ...
                    var payload = new
                    {
                        isShown = true,
                        places = places,
                        deathScreenData = deathScreenData,
                        title = "Вы умираете",
                        text = killerExists
                            ? LangFunc.GetText(LangType.Ru, DataName.YouAreDeadFromPlayer, entityKiller.Value, medics)
                            : LangFunc.GetText(LangType.Ru, DataName.YouAreDead, medics)
                    };

                    // Сериализуем и отправляем клиенту обработчик openHospitalDialogData
                    var json = JsonConvert.SerializeObject(payload);
                    Trigger.ClientEvent(player, "openHospitalDialogData", json);
                }
            }
            catch (Exception e)
            {
        ...
        // Вспомогательный метод для получения имени оружия по хешу.
        // Добавьте этот метод в класс Ems (рядом с weaponHashToString).
                ...
                switch (interact)
                {
                    ...
                    case 5:
                        ...
                        if (vehicleLocalData != null)
                        {
                            ...
                            if (medCount >= vMain.GetMaxWeight(vehicle.Model))
                            {
                            ...
                            Chars.Repository.AddNewItem(null, VehicleManager.GetVehicleToInventory(vehicle.NumberPlate), "vehicle", ItemId.HealthKit, zagruz, MaxSlots: vMain.GetMaxWeight(vehicle.Model));
