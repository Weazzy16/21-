...
const getStatsData = (json) => {
  ...
  InventoryWeight: json[54],      // Текущий вес инвентаря
		MaxInventoryWeight: json[55],   // Максимальный вес инвентаря
		BackpackWeight: json[56],       // Текущий вес рюкзака
		MaxBackpackWeight: json[57],    // Максимальный вес рюкзака
	}
}
...
let lastCheckTime = 0;
const CHECK_INTERVAL = 200; // Проверяем каждые 200мс (быстрее!)

// ✅ Используем render event (каждый кадр), но с ограничением
mp.events.add('render', () => {
    try {
        const now = Date.now();
        
        // Проверяем только раз в 200мс
        ...
        // ✅ ИСПОЛЬЗУЕМ ВСТРОЕННЫЙ МЕТОД RAGE MP!
        const lightsState = vehicle.getLightsState(1, 1);
        
        let currentState = 0;
        
        if (lightsState.highbeamsOn) {
            currentState = 2; // Дальний свет
        } else if (lightsState.lightsOn) {
            currentState = 1; // Ближний свет
        }
        ...
        if (currentState !== lastLightsState) {
            ...
            console.log('🔦 [AUTO] Lights state:', currentState);
        }
